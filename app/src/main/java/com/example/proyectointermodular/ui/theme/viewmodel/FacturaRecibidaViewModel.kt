...
class FacturaRecibidaViewModel : ViewModel() {
    ...
    private fun cargarFacturasEnTiempoReal() {
        facturaRecibidaRepository.obtenerFacturas { facturasObtenidos ->
    ...
    fun eliminarFactura(id: String) {
        viewModelScope.launch {
            try {
                ...
                facturaRecibidaRepository.eliminarFactura(id)
    ...
    fun agregarFactura(facturaRecibida: FacturaRecibida) {
        viewModelScope.launch {
            try {
                val nuevoId = facturaRecibidaRepository.agregarFactura(facturaRecibida)
                // Actualiza la factura con el nuevo ID y la añade a la lista local
                val facturaConId = facturaRecibida.copy(id = nuevoId)
